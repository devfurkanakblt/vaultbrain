...
| Incremental save acknowledgement |      < 20 ms | Met (TS); desktop met to 10k |
...
| ------------- | -------------: | -----------------: |
| 1,000 notes   |        11.8 ms |            15.1 ms |
| 10,000 notes  |         9.8 ms |            16.0 ms |
| 100,000 notes |    **13.4 ms** |    still measuring |

Before the change the same measurement gave 26.0 ms at 1,000 notes and 141.7 ms
at 10,000 for TypeScript, and 34.4 ms and 2,553 ms for the desktop core.

The TypeScript library — the CLI and MCP path — meets the budget at every tier
including 100,000 notes, the size the contract specifies. The desktop core
meets it through 10,000; its remaining cost is the durable file operations a
save still makes (write-ahead journal, archived revision, note object, log
append) rather than anything that scales with the vault. `npm run
benchmark:rust` reports it and the `performance-budgets` CI job gates it.
