...
import {
  parseDesktopSyncRequest,
  syncProgress,
  type DesktopSyncProgress,
  type DesktopSyncRequest,
} from "./desktop-sync-protocol.js";

const MAX_STDIN_BYTES = 256 * 1024;
type DeviceId = `${string}-${string}-${string}-${string}-${string}`;

function readRequest(): DesktopSyncRequest {
  const input = Buffer.alloc(MAX_STDIN_BYTES + 1);
  let length = 0;
  try {
    ...
    if (length > MAX_STDIN_BYTES) throw new Error("Desktop sync request exceeds the protocol limit.");
    return parseDesktopSyncRequest(JSON.parse(input.subarray(0, length).toString("utf8")));
  } finally {
    input.fill(0);
  }
}
...
function localDeviceId(manager: SyncDeviceManager, vaultPath: string, requested?: string): string | undefined {
    ...
    manager
      ...
      ?.body.devices.filter(
        (device) =>
          !device.revokedAt &&
          fs.existsSync(
            path.join(vaultPath, "documents", "sync", "identity", `${device.certificate.deviceId}.key.enc`),
          ),
      ) ?? [];
  if (devices.length > 1) throw new Error("Select the local device identity explicitly.");
  return devices[0]?.certificate.deviceId;
}
...
async function run(request: DesktopSyncRequest): Promise<DesktopSyncProgress> {
  switch (request.operation) {
    ...
    case "request": {
      ...
      try {
        return result(request, {
          enrollmentRequest: manager.createEnrollmentRequest(
            request.deviceName!,
            request.deviceId as DeviceId | undefined,
          ),
        });
      } finally {
        manager.close();
    ...
    case "approve": {
      ...
      const manager = new SyncDeviceManager(request.vaultPath, request.passphrase);
      try {
        const registry = manager.enroll(request.enrollmentRequest);
        return result(request, { registryRevision: registry.body.revision });
      } finally {
        manager.close();
      }
    }
    case "revoke": {
      ...
      try {
        cutoff = Math.max(
          ...
          ...log
            .changes()
            .filter((change) => change.deviceId === request.deviceId)
      ...
      const manager = new SyncDeviceManager(request.vaultPath, request.passphrase);
      try {
        const registry = manager.revoke(request.deviceId!, cutoff);
        return result(request, {
          registryRevision: registry.body.revision,
          epoch: registry.body.epoch,
          revokedAfterSequence: cutoff,
        });
      } finally {
        manager.close();
      }
    ...
    case "resolve": {
      ...
      const manager = new SyncDeviceManager(request.vaultPath, request.passphrase);
      const vault = new SyncedDocumentVault(
        request.vaultPath,
        request.passphrase,
        localDeviceId(manager, request.vaultPath, request.deviceId),
      );
      try {
        const value =
    ...
    case "push": {
      ...
      try {
        ...
        const checkpointArtifact = checkpoint
          ? await client.uploadArtifact("checkpoint", manager.exportCheckpoint())
          : null;
...
try {
  ...
  process.stdout.write(
    `${JSON.stringify({ version: 1, state: "failed", message: "Desktop sync helper rejected the operation." })}\n`,
  );
  process.exitCode = 1;
}
