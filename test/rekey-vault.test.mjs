...
test("a re-key's lock records its own stale window, and a short write honours it", () => {
  ...
  // A lock is only ever reclaimed from a process proven dead, so the record
  // this test plants must name one. It carries this test runner's own pid
  // otherwise, which is alive, and liveness alone would decide both halves
  // below — leaving the declared window, the thing under test, unexercised.
  holder = { ...holder, pid: exitedPid() };
